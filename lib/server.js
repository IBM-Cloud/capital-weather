...
function getWeatherService(appEnv) {
  ...
  if (!serviceCreds) {
    ...
    serviceCreds = appEnv.getServiceCreds("capital-weather-insights");
    if (!serviceCreds) {
      console.log("'capital-weather-insights' not bound to this application");
      console.error("No bound weather services -- please debug");
      return;
    }
    else {
      fullService = false;
      console.log("Using Weather Insights Service");
    }
  }
  else {
    fullService = true;
...
function createWeatherService(serviceCreds) {
  if (fullService) {
    ...
    var url    = serviceCreds.url;
    var port   = serviceCreds.port;
    if (url && port) return new weatherService.createBasic(url, port);
  }
}
