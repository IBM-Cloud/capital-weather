...
// Sends client which service type is being used
server.route({
  method: 'GET',
  path:'/api/serviceType',
  handler: api_getServiceType
});
...
//------------------------------------------------------------------------------
// Reqest Handler for getting weather service type
function api_getServiceType(request, reply) {
  var serviceType = {
    type: fullService
  };
  var serviceString = JSON.stringify(serviceType);
  reply(serviceString);
}
...
function api_currentConditions(request, reply) {
  ...
  wService.getCurrentConditions(lat, lon, function(result) {
    ...
    // If success, extract important results and return data
      ...
      // Adjust for TWC API results format
      if (resultJSON.observation.imperial) {
        resultJSON.observation.temp = resultJSON.observation.imperial.temp;
        resultJSON.observation.wspd = resultJSON.observation.imperial.wspd;
        resultJSON.observation.wx_icon = resultJSON.observation.icon_code;
        resultJSON.observation.wx_phrase = resultJSON.observation.phrase_32char;
        
      }
      var currentConditions = {
        iconCode:         resultJSON.observation.wx_icon,
        conditionPhrase:  resultJSON.observation.wx_phrase,
        temp:             resultJSON.observation.temp,
        windSpeed:        resultJSON.observation.wspd,
...
function getWeatherService(appEnv) {
  ...
  if (!serviceCreds) {
    console.log("'weather-service' not bound to this application");
    serviceCreds = appEnv.getServiceCreds("capital-weather-data");
    if (!serviceCreds) {
      console.log("'capital-weather-data' not bound to this application");
      console.error("No bound weather services -- please debug");
      return;
    }
    else {
      fullService = false;
      console.log("Using Weather Company Data service");
    }
  }
  else {
    fullService = true;
    console.log("Using TWC Service");
  }

  return createWeatherService(serviceCreds);
}

// Returns a weather service object based on existing weather service
function createWeatherService(serviceCreds) {
  if (fullService) {
    var url    = serviceCreds.url;
    var apiKey = serviceCreds.apiKey;
    if (apiKey && url) return new weatherService.createFull(url, apiKey);
  }
  else {
    var url    = serviceCreds.url;
    if (url) return new weatherService.createBasic(url);
  }
}
