...
function createBasicService(url) {
  return new Service(url, false);
}
...
function Service__getCurrentConditions(lat, lon, cb) {
  ...
  if (this.full) {
    ...
    console.log("Getting current conditions from Weather Company Data");
    url = this.dataUrl + "/v1/geocode/" +
          lat + "/" + lon +
...
function Service_predictConditions(lat, lon, month, day, cb) {
  ...
  async.parallel(dateReqs, function(err,results) {
    ...
    var mostFreqCond, iconCode;
    if (conditionStrings.length > 0) {
      var maxCond = 0;
      for (var j=0; j < conditionStrings.length; j++) {
        var condVal = conditions.get(conditionStrings[j]);
        if (condVal > maxCond) {
          mostFreqCond = conditionStrings[j];
          maxCond = condVal;
        }
      }
      iconCode = conditions.get(mostFreqCond + "-icon");
    }

    // Get the average temperature from the list
    var status, success, tempAvg;
    if (temps.length > 0) {
      tempAvg = 0;
      success = true;
